...
void FujiHeatPump::connect(Stream *serial, bool secondary){
    return this->connect(serial, secondary, -1, -1);
}

void FujiHeatPump::connect(Stream *serial, bool secondary, int rxPin=-1, int txPin=-1){
    _serial = serial;
//     if(rxPin != -1 && txPin != -1) {                     // вызов begin() теперь делает ПОЛЬЗОВАТЕЛЬ
// #ifdef ESP32
...
//     _serial->setTimeout(200);
    ...
    // КРИТИЧЕСКИ ВАЖНО: инициализация для Arduino Nano
        ...
        UCSR0A = 0x00; // Ускорение выключено
        UCSR0C = (1 << UCSZ01) | (1 << UCSZ00) | (1 << UPM01); // 8 бит, четный паритет
        UCSR0B = (1 << RXEN0) | (1 << TXEN0); // Включить приемник и передатчик
    #else
        // Для ESP32 и других плат
        _serial->begin(500, SERIAL_8E1);
    #endif
...
void FujiHeatPump::printFrame(byte buf[8], FujiFrame ff) {
    for (int i = 0; i < 8; i++) {
        if (buf[i] < 0x10) Serial.print('0');   // ведущий ноль
        Serial.print(buf[i], HEX);
        Serial.print(' ');
    }
    Serial.print(F(" mSrc:")); Serial.print(ff.messageSource);
    Serial.print(F(" mDst:")); Serial.print(ff.messageDest);
    Serial.print(F(" mType:")); Serial.print(ff.messageType);
    Serial.print(F(" write:")); Serial.print(ff.writeBit);
    Serial.print(F(" login:")); Serial.print(ff.loginBit);
    Serial.print(F(" unknown:")); Serial.print(ff.unknownBit);
    Serial.print(F(" onOff:")); Serial.print(ff.onOff);
    Serial.print(F(" temp:")); Serial.print(ff.temperature);
    Serial.print(F(" mode:")); Serial.print(ff.acMode);
    Serial.print(F(" cP:")); Serial.print(ff.controllerPresent);
    Serial.print(F(" uM:")); Serial.print(ff.updateMagic);
    Serial.print(F(" cTemp:")); Serial.print(ff.controllerTemp);
    Serial.print(F(" acError:")); Serial.print(ff.acError);
    Serial.println();
